# -*- coding: utf-8 -*-
#程序启动画面QSplashScreen##
#许多应用程序的主窗口在打开前都会先展示一个启动画面(例如PS)，这是因为打开需要一定时间，
#所以用启动画面来显示模块加载进度，这也是一种增加程序使用友好度的方法。
#通常我们会将程序启动画面的代码放在程序入口中，位于sys.exit(app.exec_())之前。
#下面我们就给在上一节中的记事本应用加一个启动画面
import sys
import time
from PySide.QtCore import Qt,QMimeData
from PySide.QtGui import QApplication,QMainWindow,QTextEdit,QAction,QFileDialog,\
						QMessageBox,QFontDialog,QColorDialog,QIcon,QPixmap,QSplashScreen
##python 2.6需要加下面这几行
#######################
...
if __name__ == '__main__':
	...
	splash.setPixmap(QPixmap('images/textImage/splash.jpg'))
	splash.setWindowFlags(Qt.WindowStaysOnTopHint)
	splash.show()
	splash.showMessage('Welcom to use this PySide-Made NoteBook~',
